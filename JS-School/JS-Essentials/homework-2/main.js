...
const months = [];
const averageOfDeps = {};
const averageOfMonths = {};

const getTotal = (rows) =>
  rows.reduce(function rowSum(sum, row) {
    return sum + parseFloat(row[3])
  }, 0);

const total = getTotal(data).toFixed(2);

const getUniqueItems = (array, index) => {
  data.forEach((item) => {
    if (array.indexOf(item[index]) === -1) {
      array.push(item[index]);
    }
  })
};

getUniqueItems(departments, 1);
getUniqueItems(months, 0);

const getAverageOfDeps = (data, departments) =>
  departments.forEach(function (dep) {
    const depArray = data.filter(function (item) {
      return dep === item[1];
    });
    averageOfDeps[dep] = getAverage(getTotal(depArray), depArray.length);
  });

const getAverageOfMonths = (data, months) =>
  months.forEach(function (month) {
    const monthArray = data.filter(function (item) {
      return month === item[0];
    });
    averageOfMonths[month] = getAverage(getTotal(monthArray), monthArray.length);
  });

const getAverage = (total, count) => (total / count).toFixed(2);


getAverageOfDeps(data, departments);
getAverageOfMonths(data, months);
...
function createTable(titles, data) {
  ...
  Object.keys(data).forEach(function (item) {
    ...
    // couldn't fix it better without big changes in function
    if (td.textContent === "0") {
      tr.removeChild(td);
    }
...
createTable(["Month", "Avg.Amount"], averageOfMonths);
createTable(["Department", "Avg.Amount"], averageOfDeps);
